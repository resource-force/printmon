...
const GROUP = Groups.HIGH_SCHOOL;

async function addMeterData(type: MeterTypes, data: DeviceMeterSummary[], transaction: Transaction) {
    ...
    await sequelize.transaction(async t => {
      ...
      for (const meter of METERS) {
        ...
        if (newDevices.length > 0) {
          ...
          await addMeterData(meter as unknown as MeterTypes, await fetcher.meterHistoryForDevices(
            meter,
            newDevices,
            START_DATE,
            moment(),
        ...
        const lastFetch = lastFetchRes ? moment(lastFetchRes.dataValues.lastFetch) : START_DATE;
        console.log("- Fetching all history since last fetch at", lastFetch.toLocaleString());
        await addMeterData(meter as unknown as MeterTypes, await fetcher.meterHistoryForGroup(meter, GROUP, lastFetch, moment()), t);
...
async function updateDevices(fetcher: lw.LaserWatchFetcher, transaction: Transaction) {
